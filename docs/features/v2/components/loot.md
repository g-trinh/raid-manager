...
- **All 3 of a boss's signature items share the same bonus tier** — the magnitude is determined by the boss's position in the gauntlet, not by which phase an item is derived from. A Phase 1 item and a Phase 3 item from the same boss grant identical bonuses; only the Role Lock and flavor differ between them.
...
- A Full Victory against a single boss now grants up to **3x** the per-boss total it used to (e.g., +9/+9 at the early tier instead of +3/+3) — but spread across up to 3 different roles, so no single member benefits 3x as much. A Narrow Victory grants 2x.
- A member who equips every item offered to their role across a 15-boss gauntlet gains substantially more than under the single-item model — but role-locking still naturally distributes drops across the roster's 8 members, and not every phase locks to every role with equal frequency
- A more typical outcome — a member equipping a handful of items over a run, spread across several bosses — nets a meaningful but not run-defining boost to their Skill/Liability (recall current pool averages sit around Skill 55 / Liability 45), enough to visibly shift their contribution to a role average without single-handedly guaranteeing late-phase success against targets in the 70-80 range
...
- **Tuning flag**: tripling the per-boss item count is the single biggest numeric change in this redesign. If playtesting shows roster stats inflate too quickly (especially with multiple Full Victories in a row), the first lever to pull is the magnitude curve above (e.g., flatten it to +2/+2 → +3/+3 → +4/+4), not the per-phase structure itself.
