...
- Keep the mechanical surface small: flat, permanent, stacking Skill/Liability deltas reusing the existing loot-bonus plumbing
...
| Liability Delta | int | Flat permanent adjustment to Liability, applied when the trigger condition is met |

### Personality Catalogue

| Personality | Trigger condition | Skill Delta | Liability Delta | Flavor |
|---|---|---|---|---|
| **Loner** | n/a | — | — | No reaction to anything. Pure baseline. |
| **Altruist** | Loot is granted to **another member** (not self) | — | **+10** | Happy to see the squad gear up — becomes more reliable |
| **Altruist** | Loot is granted to a **Glory Hound** | **−10** | — | Resents members who don't play for the guild — personal performance dips |
| **Glory Hound** | Loot is granted to **self** | **+10** | — | Basks in the recognition — performs better |
| **Glory Hound** | Loot is granted to **someone else** | — | **−10** | Sulks at being passed over — becomes less reliable ("loses reliability" → Liability decreases) |
...
- **At draft**: the member's personality is shown alongside their existing properties (Name, Role, Skill, Liability) — the player can see they're drafting a Glory Hound or an Altruist and weigh the risk/reward knowingly
- **Fixed for the run**: personality never changes once rolled — no hidden reveals, no redemption arcs, no escalation
- **After triggers fire**: the member's adjusted Skill/Liability is reflected wherever effective stats are shown (mirroring how equipped loot bonuses are surfaced today)
...
- **Loot to another member**: every time loot is granted to anyone other than this Altruist, their Liability **+10** (stacks across multiple grants, clamped to [0, 100])
- **Loot to a Glory Hound**: every time loot is granted to *any* Glory Hound on the roster, this Altruist's Skill **−10** (stacks, clamped to [0, 100]) — the two effects can both fire from the same grant event if the recipient is a Glory Hound (the Altruist is happy gear went to a teammate, but resentful it went to *that* teammate)

### Glory Hound

- **Loot to self**: every time this Glory Hound personally receives loot, their Skill **+10** (stacks, clamped to [0, 100])
- **Loot to someone else**: every time loot is granted to anyone other than this Glory Hound, their Liability **−10** (stacks, clamped to [0, 100])
...
- Each fired trigger applies a flat Skill and/or Liability delta to the affected member, accumulated the same way loot bonuses are: added to a per-member running total and combined with the base stat through the existing `effectiveStat` calculation
...
sequenceDiagram
    ...
    alt X is the recipient
        alt X is an Altruist
            ...
            L->>RS: Apply +10 Skill to X (basks in recognition)
        end
    end
    loop For every other roster member M ≠ X
        alt M is an Altruist
            L->>RS: Apply +10 Liability to M (happy for the squad)
            alt X is a Glory Hound
                L->>RS: Apply -10 Skill to M (resents the Glory Hound getting gear)
            end
        else M is a Glory Hound
            L->>RS: Apply -10 Liability to M (sulks at being passed over)
        end
    end
```
...
1. Player grants an item to **Marrow Vex**. Marrow Vex's Skill rises from 70 to **80** (basks in the recognition). Sera Lindwell's Liability rises from 48 to **58** (happy gear went to a teammate) but her Skill drops from 60 to **50** (resents that it went to a Glory Hound specifically). Loners are unaffected.
2. Later, the player grants an item to a Loner instead. Marrow Vex's Liability drops from 55 to **45** (sulks at being passed over). Sera Lindwell's Liability rises again, from 58 to **68** (happy for the squad — no Glory Hound penalty this time, since the recipient wasn't one).
...
- **Sign convention**: Glory Hound's "loses reliability" when passed over maps to **Liability −10** (a decrease) — high Liability is good in this system, so "losing reliability" must lower it
